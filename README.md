...
AIエージェントからChatGPTの `latest 6 pro`（GPT-6 Pro）へ、技術調査、設計判断、レビューを依頼するためのSkillです。OpenAI APIではなく、[Oracle](https://github.com/steipete/oracle)のbrowser modeとChatGPT subscriptionを使用します。
...
- CLIのモデル指定だけでは、ChatGPT UIでGPT-6とProが選ばれていることを保証できない
...
- **モデル選択を誤認しない**: 送信直前にChatGPT UIで`GPT-6`と`Pro`の両方を確認する
...
3. Oracle用Chromeで`latest 6 pro`を選び、送信する会話で世代`6`と`Pro`の両方を確認する
...
`latest 6 pro`はユーザーが指定したChatGPT UIの選択肢です。UIがモデルとモードを別々に表示する場合も、世代6とProの両方を確認します。`Latest`だけでは世代を確定せず、通常のChromeとOracle用Chromeの選択を混同しません。
...
| ChatGPT | GPT-6とProを利用できるsubscription、およびOracle用Chromeでのログイン |
...
- GPT-6とProの両方を送信直前に確認できなければプロンプトを送らない
- Oracleが失敗してもOpenAI APIや別の有料APIへ切り替えない
- Proへファイル変更、実装、外部投稿を依頼しない

## 検証できる範囲と限界

このSkillは、Oracleがbrowser modeで動作したこと、通常版Google Chromeを使用したこと、ChatGPTの会話URLを取得したこと、送信時UIでGPT-6とProが選択されていたことを確認します。
